...
${RESUME_CONTEXT_FOR_LLM}`
  ...
  // ULTRA-FAST: Transcribe + Translate + Respond in parallel
  processAudioUltraFast: publicProcedure
    .input(z.object({
      audioBase64: z.string(),
      mimeType: z.string().default("audio/webm"),
      previousContext: z.string().optional(),
    ...
    .mutation(async ({ input }) => {
      ...
      if (transcription.length < 3) {
        return { transcription: "", translation: "", answer: "", summaryPtBr: "" };
      }

      // Analyze + Translate + Generate answer in ONE LLM call
      const result = await invokeLLM({
        messages: [
          {
            role: "system",
            content: `You are an interview assistant. Analyze the transcription and provide:
1. Portuguese translation of the question
2. Best answer in English (2-3 sentences MAX, first person, no question repetition)
3. One-line PT-BR summary (max 12 words)

Return ONLY valid JSON:
{"translation":"<PT-BR translation>","answer":"<English answer>","summary":"<PT-BR summary>"}

ANSWER RULES:
- English, BRIEF 2-3 sentences MAX, 100% human natural
- First person ONLY (I, my, we)
- NO repeating question, NO filler
- Direct answer with specific metrics from resume
        ...
        response_format: {
          ...
          json_schema: {
            ...
            schema: {
              ...
              required: ["translation", "answer", "summary"],
              additionalProperties: false,
            },
          },
        },
